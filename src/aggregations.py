...
def reshape_from_oneD(one_d_vector, shape_size_dict, args=None):
    ...
    for key in shape_size_dict:
        weight_dict[key] = copy.deepcopy(
            one_d_vector[
                one_d_vector_idx : (one_d_vector_idx + shape_size_dict[key][0])
            ]
        ).reshape(shape_size_dict[key][1])
...
def aggregation(w_locals, user_weights, args, attacker_idx=None):
    ...
    if args.aggregation == "Krum":
        ...
        if args.dataset == "femnist":
            ...
            user_one_d_test = [
                item[femnist_index] for item in copy.deepcopy(user_one_d)
            ]
        else:
